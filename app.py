...
if 연봉 > 0:
    ...
    if 신용카드_사용액 > 0 or 체크카드_사용액 > 0:
        ...
        # 연금저축 한도 적용 및 초과 금액 기부금 처리
        연금저축_한도 = 9000000
        초과_연금저축 = max(0, 연금저축_IRP_납입액 - 연금저축_한도)
        연금저축_IRP_납입액 = min(연금저축_IRP_납입액, 연금저축_한도)
        기부금_납입액 += 초과_연금저축  # 초과 금액을 기부금으로 처리
        ...
        연금저축_IRP_세액공제 = 연금저축_IRP_납입액 * 0.165 if 연봉 <= 55000000 else 연금저축_IRP_납입액 * 0.132
        ...
        # 연금저축/IRP 추가 납입 조언
        if 최종_결정세액 > 0:
            부족_금액 = 최종_결정세액 / 0.165  # 연금저축/IRP 공제율 기준
            st.warning(f"결정세액을 0원으로 만들기 위해 추가로 {부족_금액:,.0f}원을 연금저축/IRP에 납입하세요.")
        else:
            st.success("결정세액이 0원입니다! 추가 납입이 필요 없습니다.")
else:
    st.warning("호봉을 입력하거나 연봉을 직접 입력하세요.")
