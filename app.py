...
if 연봉 > 0:
    ...
    # 계산 과정 출력
    st.header("최종 계산 결과")
    st.write(f"- 연봉: {연봉:,.0f}원")
    st.write(f"- 총 소득공제: {총_소득공제:,.0f}원")
    st.write(f"- 과세표준: {과세표준:,.0f}원")
    st.write(f"- 산출세액: {산출세액:,.0f}원")
    st.write(f"- 근로소득 세액공제: {근로소득공제:,.0f}원")
    st.write(f"- 기타 세액공제: {총_세액공제:,.0f}원")
    st.success(f"최종 결정세액: {최종_결정세액:,.0f}원")
